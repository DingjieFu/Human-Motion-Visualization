...
def render(motions, outdir='test_vis', device_id=0, name=None):
    ...
    if not os.path.exists(outdir + name+'.pt'): 
        ...
        torch.save(vertices, outdir + name+'.pt')
    else:
        vertices = torch.load(outdir + name+'.pt')
    ...
    imageio.mimsave(outdir + name+'.gif', out, fps=20)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--device_id', default=0, type=int, help="motion name list")
    args = parser.parse_args()

    filename_list = args.motion_list
    filedir = args.filedir
    device_id = args.device_id
    # filedir = "./smpl_render/demo/"
    # filename_list = ["000000"]
    
    for filename in filename_list:
        motions = np.load(filedir + filename + '.npy')
        print('pred', motions.shape, filename)
        render(motions, outdir=filedir, device_id=device_id, name=filename)
